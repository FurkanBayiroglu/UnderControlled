...
class _MyRoomsPageState extends State<MyRoomsPage> with SingleTickerProviderStateMixin {
  ...
  // Odanın geçmiş olup olmadığını kontrol et
  bool _isExpired(Map<String, dynamic> room) {
    final eventDate = room['eventDate'] as Timestamp?;
    if (eventDate == null) return false;
    
    // eventTime'ı parse et (örn: "14:00")
    final eventTime = room['eventTime'] as String? ?? '00:00';
    ...
    final eventDateTime = DateTime(
      eventDate.toDate().year,
      eventDate.toDate().month,
      eventDate.toDate().day,
      hour,
      minute,
    );
    
    final durationMinutes = room['durationMinutes'] ?? 120;
    final endTime = eventDateTime.add(Duration(minutes: durationMinutes));
  ...
  // Geçmiş odaları temizle
  Future<void> _clearExpiredRooms() async {
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            const SizedBox(width: 8),
            const Text('Geçmiş Odaları Temizle'),
          ],
        ),
        content: const Text('Tarihi geçmiş tüm odalardan ayrılacaksınız. Bu işlem geri alınamaz.'),
        actions: [
      ...
      if (clearedCount > 0) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('$clearedCount geçmiş oda temizlendi'),
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Text('Geçmiş oda bulunamadı'),
  ...
  Widget _buildJoinedRooms() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        // Tarihe göre sırala - geçmiş olanlar en sona
        joinedRooms.sort((a, b) {
          final aData = a.data() as Map<String, dynamic>;
          final bData = b.data() as Map<String, dynamic>;
          final aExpired = _isExpired(aData);
          final bExpired = _isExpired(bData);
          
          if (aExpired && !bExpired) return 1;
          if (!aExpired && bExpired) return -1;
          
          final aDate = (aData['eventDate'] as Timestamp?)?.toDate() ?? DateTime.now();
          final bDate = (bData['eventDate'] as Timestamp?)?.toDate() ?? DateTime.now();
      ...
      builder: (context, snapshot) {
        ...
        if (rooms.isEmpty) {
          return _buildEmptyState(
            ...
            onButtonPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const CreateRoomPage())),
          );
        }
  ...
  Widget _buildJoinedRoomCard(String roomId, Map<String, dynamic> room) {
    ...
    final isExpired = _isExpired(room);
    
    // Tarih bilgisi
    final eventDate = room['eventDate'] as Timestamp?;
    final eventTime = room['eventTime'] as String?;
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isExpired ? Colors.grey.shade400 : (isActive ? Colors.green.shade200 : Colors.grey.shade300),
            width: 1.5,
          ),
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Container(
                    ...
                    height: 50,
                    decoration: BoxDecoration(
                      color: isExpired ? Colors.grey.withOpacity(0.2) : category.color.withOpacity(0.12),
                      borderRadius: BorderRadius.circular(14),
                    ),
                    child: Center(child: Text(category.emoji, style: TextStyle(fontSize: 24, color: isExpired ? Colors.grey : null))),
                  ),
                        ...
                        Row(
                          children: [
                            ...
                            Container(
                              ...
                              child: Text(
                                isExpired ? 'Geçmiş' : (isActive ? 'Aktif' : 'Kapalı'),
                                style: TextStyle(
                                  color: isExpired ? Colors.grey : (isActive ? Colors.green : Colors.grey),
                                  fontSize: 11,
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: isExpired ? Colors.red.withOpacity(0.1) : Colors.blue.withOpacity(0.05),
                  ...
                  if (!isExpired)
                    Expanded(
                      child: OutlinedButton.icon(
                        ...
                        style: OutlinedButton.styleFrom(foregroundColor: Colors.blue, side: const BorderSide(color: Colors.blue), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
                      ),
                    ),
                  ...
                  Expanded(
                    child: OutlinedButton.icon(
                      ...
                      icon: isLeaving
                          ...
                          : Icon(isExpired ? Icons.delete : Icons.logout, size: 18),
                      label: Text(isLeaving ? 'İşleniyor...' : (isExpired ? 'Sil' : 'Ayrıl')),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isExpired ? Colors.grey.shade100 : Colors.white,
          borderRadius: BorderRadius.circular(18),
          border: Border.all(color: isExpired ? Colors.grey.shade400 : Colors.blue.shade200, width: 1.5),
          boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 12, offset: const Offset(0, 4))],
        ),
        child: Padding(
          padding: const EdgeInsets.all(16),
          child: Column(
            ...
            children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      gradient: isExpired
                          ...
                          : LinearGradient(colors: [category.color.withOpacity(0.8), category.color]),
                      borderRadius: BorderRadius.circular(14),
                      boxShadow: [BoxShadow(color: (isExpired ? Colors.grey : category.color).withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 3))],
                    ),
              ...
              if (!isExpired) const SizedBox(height: 10),
