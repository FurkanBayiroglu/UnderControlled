...
import { ChatRequest, ChatMessage, UserPreferences, Session, MOOD_CATEGORIES } from '../types';
import { v4 as uuidv4 } from 'uuid';
import { getPrompts, Language } from '../utils/prompts';

const router = Router();

// Session storage (production'da Redis kullan)
const sessions: Map<string, Session & { language: Language }> = new Map();
...
router.post('/', async (req: Request, res: Response) => {
  try {
    const { userId, sessionId, message, preferences, language } = req.body as ChatRequest & { language?: string };
    ...
    let roomRecommendations: any[] = [];

    if (shouldShowRooms) {
      roomRecommendations = await generateRecommendations(
        session,
        userId,
        lang
      );
...
router.post('/mood', async (req: Request, res: Response) => {
  try {
    ...
    const analysis = await groqService.analyzeMood(message, lang);
 ...
 */
function checkIfUserWantsRooms(message: string, lang: Language): boolean {
 ...
 */
function detectCategoriesFromMessage(message: string, lang: Language): string[] {
  const lower = message.toLowerCase();
...
async function generateRecommendations(
  ...
  try {
    ...
    if (session.preferences.mood && categories.length === 0) {
      categories = MOOD_CATEGORIES[session.preferences.mood] || [];
    }

    // Daha önce önerilmemiş odaları al
    const rooms = await firestoreService.getActiveRoomsExcluding(
      session.recommendedRoomIds,
      {
    ...
    // Skorla ve hazırla
    const recommendations = await scoreAndPrepareRooms(
      rooms.slice(0, 8),
      session,
      session.recommendedRoomIds,
      lang
 ...
 */
async function scoreAndPrepareRooms(
  rooms: any[],
  session: Session,
  ...
  const explanations = await groqService.generateRoomExplanations(rooms, preferences, lang);
 ...
 * Eşleşme nedeni
 */
function getMatchReason(room: any, preferences: UserPreferences, lang: Language): string {
  if (lang === 'tr') {
    if (preferences.categories?.includes(room.category)) {
