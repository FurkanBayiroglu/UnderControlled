...
import { ChatMessage, ChatResponse, UserPreferences, CATEGORIES } from '../types';
...
class GroqService {
   ...
   * Mood analizi
   */
  async analyzeMood(userMessage: string, language: Language = 'tr'): Promise<{
    mood: string;
   ...
   */
  async generateRoomExplanations(
    rooms: any[],
    userPreferences: UserPreferences,
    language: Language = 'tr'
    ...
    const roomList = rooms.map((r, i) => 
      `${i + 1}. "${r.name}" (${r.category}) - ${r.participantCount}/${r.maxParticipants} ${language === 'tr' ? 'kişi' : 'people'}`
    ).join('\n');
...
- Write in ENGLISH`;
   ...
   * Varsayılan açıklama
   */
  private getDefaultExplanation(room: any, index: number, language: Language): string {
    const emoji = CATEGORIES[room.category]?.emoji || '✨';
   ...
   * Response analizi
   */
  private analyzeResponse(text: string, messages: ChatMessage[], language: Language): {
    intent: string;
    categories: string[];
