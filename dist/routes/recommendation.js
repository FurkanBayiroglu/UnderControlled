...
const userRecommendationHistory = new Map();
/**
 * POST /api/recommend
 * Oda önerileri al
 */
router.post('/', async (req, res) => {
    try {
        const { userId, preferences, limit = 5 } = req.body;
        if (!userId) {
        ...
        if (preferences?.mood && categories.length === 0) {
            categories = types_1.MOOD_CATEGORIES[preferences.mood] || [];
        }
        // Odaları çek - daha önce önerilmeyenleri tercih et
        let rooms = await firestoreService_1.firestoreService.getActiveRoomsExcluding(previouslyRecommended, {
        ...
        // AI açıklamaları al
        const explanations = await groqService_1.groqService.generateRoomExplanations(topRooms, preferences || {});
        // Önerileri hazırla
...
router.get('/categories', async (_req, res) => {
    try {
        const counts = await firestoreService_1.firestoreService.getRoomCountByCategory();
        const categories = Object.entries(types_1.CATEGORIES).map(([id, info]) => ({
            id,
            name: info.name,
            emoji: info.emoji,
...
router.post('/quick', async (req, res) => {
    try {
        ...
        const rooms = await firestoreService_1.firestoreService.getActiveRooms({
 ...
 */
function scoreRooms(rooms, preferences, previouslyRecommended) {
    return rooms.map(room => {
        ...
        if (preferences.groupSize) {
            ...
            if (preferences.groupSize === 'solo' && max <= 5)
                score += 10;
            else if (preferences.groupSize === 'small' && max <= 10)
                score += 10;
            else if (preferences.groupSize === 'large' && max >= 15)
                score += 10;
        }
        // Aktif oda bonusu (+15)
 ...
 * Eşleşme nedeni
 */
function getMatchReason(room, preferences) {
    if (preferences.categories?.includes(room.category)) {
 ...
 * Varsayılan açıklama
 */
function getDefaultExplanation(room) {
    const category = types_1.CATEGORIES[room.category];
 ...
 * Sonuç mesajı
 */
function generateResultMessage(count, preferences) {
    if (count === 0) {
